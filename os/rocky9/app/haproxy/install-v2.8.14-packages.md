...
| VIP | `211.43.190.100` |
| HTTP Listen | `211.43.190.100:80` |
| HTTPS Listen | `211.43.190.100:443` |
...
```bash
echo "1.2.3.4" >> /etc/haproxy/blocked_ips.lst
```
...
```bash
openssl s_client -connect 211.43.190.100:443 -tls1_2 -cipher 'ECDHE+AESGCM'
```
...
```bash
curl -k -i -H "X-Forwarded-For: 1.2.3.4" https://211.43.190.100/
```
...
- HAProxy는 VIP 211.43.190.100:80, 443에 바인딩
```
