...
```bash
git clone git@gitlab.plura.internal:config/filter.git
cd filter
```
...
```bash
git clone git@s-gitops:config/filter.git s-filter
cd s-filter
```
...
```bash
git remote add dev git@d-gitlab:config/filter.git
git fetch dev main
```
...
```bash
git remote add live git@gitops:config/filter.git
git fetch live
```
