...
find "$INPUT_DIR" -type f -name "*.md" | while read -r file; do
    ...
    # Define the table of contents (TOC) using `pandoc`
    toc=$(pandoc "$file" -f markdown --toc --template=plain)
    ...
    # Insert the converted HTML content and TOC into placeholders
    awk -v content="$content" -v toc="$toc" '
    {
        gsub(/{{post_content}}/, content);
        gsub(/{{toc}}/, toc);
        print;
