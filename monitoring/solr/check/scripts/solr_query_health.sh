...
# - Verbose: OK도 모두 출력
# Usage:
#   ./solr_query_health.sh [-q|--quiet] [-v|--verbose] [<config_file>]
#   ./solr_query_health.sh --config=/path/to/solr_config-041069.conf
...
for arg in "${@:-}"; do
  case "$arg" in
    ...
    * )           CONFIG_FILE="$arg" ;;   # first non-flag arg as config path
  esac
done
